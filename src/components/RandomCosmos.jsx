...
import { createCosmosScene } from '../lib/cosmosScene';
import { hasWebGL, prewarmThree } from '../lib/threeKit';
import { loadRollArt } from '../lib/rollArt';
import { flyToPlayer } from '../lib/rollHandoff';

/**
 * Анимация случайного трека «Космос»: полёт сквозь туннель обложек.
 *
 * Сцена — three.js на WebGL (src/lib/cosmosScene.js), фазы приходят из стора.
 ...
 */
export default function RandomCosmos() {
  const roll = useStore((s) => s.dice);
  const skip = useStore((s) => s.skipDice);

  const active = roll?.kind === 'cosmos';
  ...
  useEffect(() => {
    if (useStore.getState().settings.rollAnim !== 'cosmos') return undefined;
  ...
  useEffect(() => {
    ...
    createCosmosScene(canvas, { accent: accentRef.current }).then((scene) => {
      ...
      if (import.meta.env.DEV) window.__cosmos = scene;   // доступ из DevTools, как window.__store
    ...
    return () => {
      ...
      if (import.meta.env.DEV && scene && window.__cosmos === scene) window.__cosmos = null;
    };
  }, [active]);
  ...
  /* ---------- перелёт: обложка из портала → обложка плеера ---------- */
  useLayoutEffect(() => {
  ...
  const caption = phase === 'balance' ? 'Ищем что-нибудь стоящее…' : 'Летим сквозь библиотеку…';

  return (
    <div
      className={`cos-overlay ph-${phase}`}
      style={{ '--accent': accent }}
      onClick={skip}
      role="status"
      aria-live="polite"
    ...
    >
      <div className="cos-veil" />
      <canvas ref={canvasRef} className="cos-canvas" />
      <div className="cos-vignette" />
      <div className="cos-flash" />
      <div className="cos-grain" />

      {phase === 'expand' && coverSrc && (
        <img ref={flyRef} className="cos-fly" src={coverSrc} alt="" draggable={false} />
      )}

      <div className="cos-caption">
        {named ? (
          <>
